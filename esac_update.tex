...
\subsubsection{Chat widget and worker}
\label{subsub:chat}
...
ChatWorker(llm_service, knowledge_manager, message,
           context, conversation_history=None, icl=True)
\end{verbatim}

Behavior:
\begin{itemize}
  \item Runs in a background \texttt{QThread} to avoid blocking the GUI.
  \item Uses \texttt{llm\_service.generate\_response\_stream(...)} to stream response chunks back to the UI via the \texttt{response\_chunk} signal.
  \item On completion, emits \texttt{usage\_info} if token/cost usage data is available, followed by \texttt{finished}.
\end{itemize}
...
Key fields:
\begin{itemize}
  \item \texttt{last\_script\_search\_results}: list --- stores the most recent search result objects (each is a dict containing \texttt{filepath}, \texttt{filename}, \texttt{folder}, \texttt{match\_types}, \texttt{matches}, and \texttt{mtime}).
  \item \texttt{last\_search\_term}: \texttt{str} --- the last normalized search term used to populate results.
\end{itemize}

Important methods and behaviors:
\begin{itemize}
  \item \texttt{handle\_user\_message(self, message)}: entry point for text submitted by the user. Implements a creation-vs-search heuristic: if the message contains creation verbs such as ``write'', ``generate'', or ``create'', the message is forwarded to the LLM for code generation. Otherwise, a keyword extraction is performed and a script search may be initiated.
  \item \texttt{extract\_search\_keywords(self, message)}: uses the \texttt{LLMService} to extract one or more normalized keywords suitable for filesystem/script searches. The extractor uses examples and a constrained prompt to avoid ambiguous outputs (e.g., returning the actual IPTS number rather than the token \texttt{ipts}).
  \item \texttt{display\_search\_results(self, results, max\_show=15)}: accepts a list of result dicts and renders up to \texttt{max\_show} numbered entries in the chat. When ``find all'' is requested, the method returns the full list (index + full path) and updates \texttt{last\_script\_search\_results} so that \texttt{load script N} commands can reference any item returned.
  \item \texttt{load\_script\_by\_index(self, index)}: given a 1-based index referencing \texttt{last\_script\_search\_results}, reads the corresponding file from disk and emits \texttt{copy\_to\_editor\_signal} with the file contents so the \texttt{EditorWidget} receives it. Errors (index out of range, file read failure) are reported back into the chat as user-visible messages.
\end{itemize}

Developer note: the \texttt{ChatWidget} uses a short heuristic to determine whether a user intent is to ``create'' new content (send to the LLM) or to ``find'' existing scripts (run a file-system search). This reduces accidental past-script loading when the user explicitly asks for a generated example.

\subsubsection{Knowledge manager}
\label{subsub:km}

\textbf{Function:} \texttt{search\_past\_scripts(query, base\_dir="/home/controls/var/tmp", max\_results=15)}

Signature and return:
\begin{verbatim}
search_past_scripts(
    query: str,
    base_dir: str = "/home/controls/var/tmp",
    max_results: int = 15
) -> List[Dict]
\end{verbatim}
...
\begin{itemize}
  \item \texttt{filepath} (str): absolute path to the matching file
  \item \texttt{filename} (str): base filename
  \item \texttt{folder} (str): containing folder name (useful for IPTS/user folder matches)
  \item \texttt{match\_types} (List[str]): types of matches found such as \texttt{['filename','folder','content']}
  \item \texttt{matches} (List[str]): snippets or matched strings found in the file or folder path
  \item \texttt{mtime} (float): file modification time (seconds since epoch) used for recency sorting
\end{itemize}

Key behavior and implementation notes:
\begin{itemize}
  \item The function performs a depth-limited directory traversal (depth limit = 5) to avoid scanning unrelated system paths.
  \item Subfolder and IPTS-pattern matching are explicitly supported: queries like \texttt{34382\_yifan} or \texttt{34382} match folder names even when not present in the filename.
  \item Matches are collected across the full traversal, annotated with \texttt{mtime}, sorted by recency (newest first), and truncated to \texttt{max\_results}.
  \item This ordering ensures that the most recent relevant scripts are presented to the user rather than an arbitrary early subset.
\end{itemize}
